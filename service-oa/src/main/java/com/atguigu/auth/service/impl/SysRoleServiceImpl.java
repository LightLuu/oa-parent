...
//        }
    ...
    public Map<String, Object> findRoleDataByQuestionId(Long userId){
        //1 查询所有角色，返回list集合，返回
        List<SysRole> allRoleList =
                baseMapper.selectList(null);

        //2 根据userid查询 角色用户关系表，查询userid对应所有角色id
        LambdaQueryWrapper<QuestionRole> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(QuestionRole::getQuestionId,userId);
        List<QuestionRole> existUserRoleList = questionRoleService.list(wrapper);
    ...
    //4.为问卷分配所属id
    public void doAssignByQuestionId(AssginRoleVo assginRoleVo){
        //把用户之前分配角色数据删除，用户角色关系表里面，根据userid删除
        LambdaQueryWrapper<QuestionRole> wrapper = new LambdaQueryWrapper<>();
        wrapper.eq(QuestionRole::getQuestionId,assginRoleVo.getUserId());
        questionRoleService.remove(wrapper);
        ...
        for(Long roleId:roleIdList) {
            ...
            QuestionRole questionRole = new QuestionRole();
            questionRole.setQuestionId(assginRoleVo.getUserId());
            questionRole.setRoleId(roleId);
